/* ============================================================
   KAZI — ORFAB BY ORIENT CASE STUDY
   ...
   Identical logic to work-rab.js.
   ============================================================ */
(() => {
  ...
  /* ---------- Sub-hero reel ----------
     Nothing was starting this one, so it sat on its first frame and read as a
     still. Play it only while the intro is on screen, muted (the reels below
     own the audio) — same in-view rule the reel strip uses. */
  const subHero = document.getElementById('orfabSubHero');
  if (subHero) {
    subHero.muted = true; /* unattended autoplay is only allowed muted */
    new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting) safePlay(subHero);
      else subHero.pause();
    }, { threshold: 0.15 }).observe(subHero);
  }
})();
