/* ============================================================
   ...
   Entry fade-up after the card-to-hero transition, the muted
   feature video, and one slideable reel strip: drag or tap a
   side reel to re-centre; the centre reel plays WITH audio once
   the section scrolls into view; tapping it toggles pause.
   ============================================================ */
(() => {
  ...
  if (feature) {
    feature.muted = true; // no audio on this one, ever
    const play = () => safePlay(feature);
    play();
    feature.addEventListener('canplay', play);
    feature.addEventListener('pause', play); // resume if the browser throttles it
    document.addEventListener('visibilitychange', () => { if (!document.hidden) play(); });
  }

  /* ---------- Slideable reel strip ---------- */
  ...
  if (row && typeof gsap !== 'undefined') {
    ...
    /* First time the section is scrolled into view: start playback,
    ...
    io.observe(row.closest('.cs__reels'));
